...
import { addJourney } from "../../store/features/timeline/timelineSlice";

const AddTimeLinePopup = ({ onClose }) => {
  ...
  const handleAddTimeline = async (e) => {
    ...
    try {
      await dispatch(addJourney(timelineInfo)).unwrap();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 flex-wrap">
        <button type="button" onClick={onClose} className="text-sm py-2 px-4 text-gray-600 border cursor-pointer border-gray-300 rounded-lg font-semibold transition hover:bg-gray-50">
          Cancel
        </button>
        <button 
...
export default AddTimeLinePopup;